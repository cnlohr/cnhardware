...
MINIRV32_DECORATE int32_t MiniRV32IMAStep(struct MiniRV32IMAState* state, uint8_t* image, uint32_t vProcAddress,
    ...
    if ((CSR(mip) & (1 << 7)) && (CSR(mie) & (1 << 7) /*mtie*/) && (CSR(mstatus) & 0x8 /*mie*/))
    {
        ...
        for (int icount = 0; icount < count; icount++)
        {
            ...
            else
            {
                ...
                // printf( "PC: %08x IR: %08x  %08x / s0:%x s1:%x a0:%x/%x/%x/%x/%x/%x\n", ofs_pc, ir, REG(2), REG(8),
                    ...
                    case 0x73: // Zifencei+Zicsr  (0b1110011)
                    {
                        ...
                        else if (microop == 0x0) // "SYSTEM" 0b000
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    case 0:
                                        trap = (CSR(extraflags) & 3)
                                                   ? (11 + 1)
                                                   : (8 + 1); // ECALL; 8 = "Environment call from U-mode"; 11 =
