...
int main()
{
	...
	funPinMode( PIN_PCK,   GPIO_CFGLR_OUT_50Mhz_PP );
	funPinMode( PIN_VS,    GPIO_CFGLR_IN_FLOAT );
	funPinMode( PIN_HS,    GPIO_CFGLR_OUT_50Mhz_PP );

	printf( "Chip: %02x%02x\n", CamReadReg( 0x300A ), CamReadReg( 0x300B ) );
	while(1)
	{
		funDigitalWrite( PIN_HS, 0 );
		Delay_Ms(20);
		funDigitalWrite( PIN_HS, 1 );
		int i;
		for( i = 0; i < 1000000; i++ )
		{
			funDigitalWrite( PIN_PCK, 0 );
			DelaySysTick(5);
			funDigitalWrite( PIN_PCK, 1 );
			DelaySysTick(5);
		}
		//Delay_Ms( 250 );
...
void ConfigureCamera()
{
	...
	struct CamCmd sccbs[] = {
		...
		//{0x460C, 0x21}, Already tried both ways, neither works for manual control.
		...
		{0x3017, 0x0f},  // Pad output control, FREX = 0, vsync, href, pclk outputs. D9:6 enable.
		{0x3018, 0xfc},  // Pad output enable, D5:0 = 1.  GPIO0/1 = off.

		//{0x470b, 0x0f},  //VSYNC WIDTH PCLK UNIT -- does this matter, by default 0x01
		{0x4404, 0x24}, // JPEG CTRL04, normally 0x24, But, Enable gated clock = 1
