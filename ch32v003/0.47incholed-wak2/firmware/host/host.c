...
int poiofs( int scale )
{
	//return 0;
	//return ((rand()%scale)+(rand()%scale))/2;
	return rand()%scale;
}
...
reconnect:
	hid_device * hd = hid_open( USB_VENDOR_ID, USB_PRODUCT_ID, 0); // third parameter is "serial"
	if( !hd )
	{
	...
	for( j = 0; ; j++ )
	{
		...
		XShmGetImage(display, capturewindow, image, offset_x, offset_y, AllPlanes);
		...
		for( y = 0; y < screen_h; y+=8 )
		{
			for( x = 0; x < screen_w; x++ )
			{
				...
				for( row = 0; row < 8; row++ )
				{
					...
					int yofs = poiofs(scale);
					int xofs = poiofs(scale);
					uint32_t d = imagedata[screen_w*((ry*scale)+yofs)*scale+rx*scale+xofs];
					int b = d&0xff;
					int g = (d>>8)&0xff;
					int r = (d>>16)&0xff;
